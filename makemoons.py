...
from DANN_test.DANN_shallow_implementation import DANN
...
def main():
    ...
    # print(f"Xall shape is:{Xall.shape}")
...
def draw_trans_data(X, y, Xt, predict_fct=None, neurons_to_draw=None, colormap_index=0, special_points=None,
    ...
    if X is not None:
        for i in range(len(y)):
            if y[i] == 1:
                pyplot.annotate("-", X[i, :], color="green", size=50 * 1.5, textcoords='offset points',
                                xytext=(-6 * 1.5, -13 * 1.5))
            else:
                pyplot.annotate("+", X[i, :], color="red", size=30 * 1.5, textcoords='offset points',
                                xytext=(-8 * 1.5, -8 * 1.5))
